...
def main():
    ...
    try:
        ...
        emit(f"  读到 {agg['dailies_loaded']} 份日报 → 疫苗{len(events['vaccine'])} 合成生物{len(events['synbio'])} 交叉{len(events['cross'])}")
        ...
        if agg["dailies_loaded"] < args.days * 0.5:
            emit(f"⚠️ 仅 {agg['dailies_loaded']}/{args.days} 份日报归档,事件/融资/监管池可能偏薄(Cowork 侧酌情 WebSearch 补)")
    except Exception as e:
